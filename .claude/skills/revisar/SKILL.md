...
- **Contratos de funciones**: Si se cambió la firma de un servicio/action, buscar TODOS los call sites con `Grep` y confirmar compatibilidad
- **Prisma queries**: Si se modificó un `select`/`include`, verificar que los consumidores no acceden a campos eliminados
...
- **Componentes reutilizados**: Si se modificaron props, buscar todos los usos y verificar
...
| Timezone | Fechas en UTC en BD, UY en UI. Usar `date-utils.ts` |
| Auth | Verificar rol del usuario en actions protegidas |
...
- Página pública (ranking, fixture, perfil jugador)
...
| **RISKY** | Cambios que podrían causar regresiones en producción. Explicar el riesgo específico |
