...
interface MatchResultEditedEmailProps {
  adminName: string
...
MatchResultEditedEmail.PreviewProps = {
  adminName: 'Mati',
  ...
  score: '4-6  6-7(3)',
  isWalkover: false,
} satisfies MatchResultEditedEmailProps

export default function MatchResultEditedEmail({
  adminName,
  tournamentName,
  categoryName,
  groupNumber,
      ...
      <Body style={{ backgroundColor: theme.colors.background, fontFamily: theme.fonts.sans, padding: '20px 0' }}>
        <Container style={{ backgroundColor: theme.colors.cardBg, borderRadius: '8px', padding: '40px', maxWidth: '480px', margin: '0 auto', border: `1px solid ${theme.colors.border}` }}>
          ...
          <Text style={{ fontSize: '14px', color: theme.colors.text, margin: '0 0 16px' }}>
            El resultado del partido fue editado por <strong>{adminName}</strong>.
          </Text>
